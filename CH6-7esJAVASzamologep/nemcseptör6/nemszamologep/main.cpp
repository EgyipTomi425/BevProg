...
constexpr char gyokok = 's';
constexpr char hatvanyok = 'p';
constexpr char result = '=';
const string declkey = "let"; //constexpr string since C++20 only
const string gyok="sqrt";
const string hatvany="pow";
	...
	switch (ch)
	{
    	...
    	default:
    	{
    		if (isalpha(ch))
    		{
    			...
    			if (s == gyok) return Token{gyokok};
    			if (s==hatvany) return(hatvanyok);
...
int main(){
try {
cout<<"Szia Uram! + - * / pi e sqrt pow ( ) % erdekel?"<<endl;
